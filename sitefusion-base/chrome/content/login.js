...
SiteFusion.Login = {
    ...
    DownloadExtension: function(id, targetFile) {
        ...
        try {
            ...
            if (targetFile.exists() && targetFile.isFile() && targetFile.isWritable()) {
            ...
            var persistObject = Cc["@mozilla.org/embedding/browser/nsWebBrowserPersist;1"].createInstance(Ci.nsIWebBrowserPersist);
            var nsIWBP = Ci.nsIWebBrowserPersist;

            // Set some settings
            persistObject.progressListener = progressListener;
            persistObject.persistFlags = nsIWBP.PERSIST_FLAGS_REPLACE_EXISTING_FILES |
            ...
            if (parseInt(info.platformVersion) < 36) {
                persistObject.saveURI(uri, null, null, null, null, targetFile, privacyContext);
            } else {
                persistObject.saveURI(uri, null, null, null, null, null, targetFile, privacyContext);
            }
